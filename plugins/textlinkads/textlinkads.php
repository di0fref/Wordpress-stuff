...
function tla_admin_notices() 
{
    ...
    if ($textlinkads_object->websiteKey) {
        return;
    }
...
{
    var $websiteKey = '32KDHJ3679BXH7JI5UKV';
    ...
    function textlinkadsObject()
    {
        ...
        $this->siteKeys = get_option('tla_site_keys');
        if (!is_array($this->siteKeys) || count($this->siteKeys) == 0) {
...
RewriteRule $ /index.php/404
    ...
    function initialize()
    {
        ...
        if (count($this->websiteKeys) > 1) {
            ...
            if ($pageKey) {
                $where = "WHERE xml_key='" . mysql_real_escape_string($pageKey) . "'";
            } else {
                $where = "WHERE 1=0";
            }
        } else {
        ...
        $this->ads = $wpdb->get_results("SELECT * FROM " . $this->DataTable . " $where");
        if (!is_array($this->ads)) return;
    ...
    function updateLocalAds()
    {
        ...
        foreach ($this->websiteKeys as $url => $key) {
            $url = 'http://www.text-link-ads.com/xml.php?k=' . $key . '&l=wordpress-tla-3.9.6';
            ...
            if ($xml = $this->fetchLive($url)) {
                ...
                if ($links && is_array($links)) {
                    $query = "INSERT INTO `" . $this->DataTable . "` (`url`, `post_id`, `xml_key`, `text`, `before_text`, `after_text`) VALUES ";
                    
                    foreach ($links as $link) {
                        $query .= " (
                            '" . mysql_real_escape_string($link['URL']) . "',
                            '" . mysql_real_escape_string(isset($link['PostID']) ? $link['PostID'] : 0) . "',
